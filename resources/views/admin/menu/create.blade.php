                ...
                <div class="panel-body">
                    ...
                    <form class="form-horizontal" id="form">
                        ...
                        <div class="form-group">
                            ...
                            <div class="col-md-10">
                                <input type="text" class="form-control required" name="url" placeholder="如:admin/orders">
                            </div>
                        </div>
                        <div class="form-group" id="icon-form-group">
                            <label class="col-md-2 control-label">菜单图标</label>
                            <div class="col-md-6">
                                <input type="text" id="icon-input" class="form-control required" name="icon" placeholder="如:mdi mdi-access-point">
                            </div>
                            <div class="col-md-2">
                                <button type="button" id="icon-view" class="btn btn-success waves-effect waves-light">预览</button>
                            </div>
                            {{--<div class="col-md-2 align-content-center">
                                <button type="button" id="icon-view" class="btn btn-dark waves-effect waves-light"><i class="132"></i></button>
                            </div>--}}
                        </div>
                        <div class="form-group">
                            <label class="col-md-2 control-label">权重,值越大排序越靠前</label>
                            <div class="col-md-10">
                                <input type="text" class="form-control required" name="sort" placeholder="权重">
                            </div>
                        </div>
                        <div class="form-group">
                                ...
                                <select class="form-control" name="pid">
                                    @if(!empty($p_menus))
                                        ...
                                        @foreach($p_menus as $p_menu)
                                            <option value="{{$p_menu->id}}">{{$p_menu->name}}</option>
                                        @endforeach
                                    @else
                                        <option value="0">顶级菜单</option>
                                    @endif
                            ...
                            <div class="col-md-10">
                                ...
                                @foreach($roles as $role)
                                    <div class="checkbox checkbox-primary" style="float: left;margin-left: 20px;">
                                        <input id="checkbox2" type="checkbox" class="role" value="{{ $role->id }}">
                                        <label for="checkbox2">
                                            {{$role->name}}
                                        </label>
                                    </div>
...
@section('js')
    <script>
        ...
        function commit(){
            ...
            data.roles = roles;
            myRequest("/admin/system/menu/store","post",data,function(res){
                console.log(res);
                layer.msg(res.msg);
                if(res.code == 200){
                    setTimeout(function(){
